...
indexHtml = TL.concat
    ...
    , "  <title>Dominó - Haskell</title>"
    , "  <style>", cssStyles, indexCssStyles, "</style>"
    , "</head>"
    , "<body>"
    , "  <div class='container' id='main-menu'>"
    , "    <div class='title-container'>"
    , "      <div class='domino-piece'>🁣</div>"
    , "      <h1 class='game-title'>"
    ...
    , "      <button onclick=\"showGameSetup()\" class='btn-start'>▶️ Jugar</button>"
    , "      <button onclick=\"exitGame()\" class='btn-exit'>❌ Salir</button>"
    , "    </div>"
    , "  </div>"
    , ""
    ...
    , "          <button onclick=\"selectGameMode('robadito')\" id='btn-robadito' class='btn-selector active'>🎴 Robadito</button>"
    , "          <button onclick=\"selectGameMode('norobadito')\" id='btn-norobadito' class='btn-selector'>🚫 Sin Robar</button>"
    , "        </div>"
    , "      </div>"
    , ""
    , "      <!-- Modo VS -->"
    ...
    , "        <h3>Jugadores</h3>"
    , "        <div class='selector-group'>"
    , "          <button onclick=\"selectVsMode('1vs1')\" id='btn-1vs1' class='btn-selector'>👤 1 vs 1</button>"
    , "          <button onclick=\"selectVsMode('freeforall')\" id='btn-freeforall' class='btn-selector active'>👥 Todos vs Todos</button>"
    , "          <button onclick=\"selectVsMode('2vs2')\" id='btn-2vs2' class='btn-selector'>👥👥 2 vs 2</button>"
    , "        </div>"
    , "      </div>"
    , ""
    ...
    , "        <div class='selector-group'>"
    , "          <button onclick=\"selectDifficulty('easy')\" id='btn-easy' class='btn-selector btn-diff-easy active'>😊 Fácil</button>"
    , "          <button onclick=\"selectDifficulty('medium')\" id='btn-medium' class='btn-selector btn-diff-medium'>🤔 Medio</button>"
    , "          <button onclick=\"selectDifficulty('hard')\" id='btn-hard' class='btn-selector btn-diff-hard'>😈 Difícil</button>"
    , "          <button onclick=\"selectDifficulty('extreme')\" id='btn-extreme' class='btn-selector btn-diff-extreme'>🔥 Extremo</button>"
    , "        </div>"
    , "      </div>"
    , ""
    , "      <div class='setup-actions'>"
    , "        <button onclick=\"startGame()\" class='btn-start-game'>🚀 Iniciar Partida</button>"
    , "        <button onclick=\"backToMenu()\" class='btn-back'>⬅️ Volver</button>"
    , "      </div>"
    , "    </div>"
    , "  </div>"
    , ""
...
gameHtml = TL.concat
    ...
    , "        <div class='mini-domino-piece'>🁣</div>"
    ...
    , "          <h3>Acciones</h3>"
    , "          <button id='btn-draw' class='btn btn-action' onclick='drawTile()'>🎴 Robar</button>"
    , "          <button id='btn-pass' class='btn btn-action' onclick='passTurn()'>⏭️ Pasar</button>"
    , "          <button class='btn btn-new' onclick='newGame()'>🔄 Nueva Partida</button>"
    , "          <button class='btn btn-menu' onclick='goToMenu()'>🏠 Menú</button>"
    , "        </div>"
    , "      </div>"
    , "    </div>"
    , "    "
    , "    <div id='side-selector' class='side-selector hidden'>"
    , "      <p>¿En qué lado quieres jugar?</p>"
    , "      <button class='btn' onclick='playSelected(\"left\")'>⬅️ Izquierda</button>"
    , "      <button class='btn' onclick='playSelected(\"right\")'>➡️ Derecha</button>"
    ...
    , "        <button id='btn-next-round-modal' class='btn btn-continue hidden' onclick='nextRound()'>▶️ Siguiente Ronda</button>"
    , "        <button id='btn-new-game-modal' class='btn btn-new' onclick='newGame()'>🔄 Nueva Partida</button>"
    , "        <button class='btn btn-menu' onclick='goToMenu()'>🏠 Menú</button>"
    , "      </div>"
    , "    </div>"
    , "  </div>"
...
jsScript = TL.unlines
    ...
    , "  "
    , "  // Si el tablero está vacío, jugar automáticamente"
    ...
    , "        <div class='team-label'>🔴 Equipo Rival</div>"
    , "        ${teamB.map((p, i) => `"
    ...
    , "    emptyMsg.textContent = 'Mesa vacía - Juega la primera ficha';"
    ...
    , "  }"
    , "  "
    , "  // Mano del jugador (según control seleccionado)"
    ...
    , "  console.log('Índice de mano:', handIndex);"
    ...
    , "    } else if (isDataA100) {"
    , "      modalTitle.textContent = '🎯 ¡Ronda Terminada!';"
    , "    } else {"
    ...
    , "    "
    , "    // Mostrar resumen de puntuación si es Data a 100"
    ...
    , "      `;"
    , "      "
    , "      // Mostrar botón de siguiente ronda solo si la partida no terminó"
    , "      if (matchOver) {"
    , "        btnNextRound.classList.add('hidden');"
    , "        btnNewGame.textContent = '🔄 Nueva Data a 100';"
    , "      } else {"
    ...
    , "  "
    , "  const bonusInfo = [];"
    , "  if (ms.msjBonusNoSalida) bonusInfo.push('🚫 No salida +20');"
    , "  if (ms.msjBonusPrimerDoble && ms.msjIsFirstRound) bonusInfo.push('🥇 1er partido x2');"
    , "  if (ms.msjBonusCapicua) bonusInfo.push('🔄 Capicúa x2');"
    , "  "
    , "  let html = `"
    , "    <h3>🎯 Data a ${ms.msjTargetScore}</h3>"
    ...
    , "    html += `<button class='btn-next-round' onclick='nextRound()'>▶️ Siguiente Ronda</button>`;"
    , "  }"
    , "  "
    ...
    , "setInterval(async () => {"
    , "  // Solo ejecutar autoplay cuando control está en '0' (modo normal)"
